...
@Component({
  ...
  styleUrl: './dashboard-assessment-findings.component.scss'
})
export class DashboardAssessmentFindingsComponent implements OnInit {
  ...
  bindAssessmentFindingInputs() {
    ...
    this.assessmentFindingInputs.findinG_STATUS = !this.assessmentFindingInputs.findinG_STATUS ? "Open" : this.assessmentFindingInputs.findinG_STATUS;
    this.assessmentFindingInputs.findinG_TYPE = this.assessmentFindingInputs.findinG_TYPE == undefined || this.assessmentFindingInputs.findinG_TYPE.length === 0 ? ["Weakness", "Threat"] : this.assessmentFindingInputs.findinG_TYPE;
    ...
    this._appService.getAccountsForCSATDashboard(this.allCust).subscribe(data => {
      ...
      if (this.customer != undefined) {
        if (this.allCust) {
          ...
          this.customeR_IDS = this.customer.map((x: any) => x.cusT_ID);
  ...
  loadData() {
    if (this.assessmentFindingInputs.charT_TITLE == "") {
      this._util.showWarningPopup('Chart title should not be empty.');
      return;
    }
    if (this.assessmentFindingInputs.xAxis == "") {
      this._util.showWarningPopup('Please select x axis');
      return;
    }
    if (this.assessmentFindingInputs.yAxis == "") {
      this._util.showWarningPopup('Please select Y axis');
      return;
    }
    this.assessmentFindingInputs.customeR_IDS = this.getCustomerIds();
    this.assessmentFindingInputs.StarT_DATE = new Date(this.fromDate!).toDateString();
  ...
  ViewAssessmentFindingDetails() {
    // Ensure customer IDs are set before opening dialog
    this.assessmentFindingInputs.customeR_IDS = this.getCustomerIds();
    this.assessmentFindingInputs.StarT_DATE = new Date(this.fromDate!).toDateString();
    this.assessmentFindingInputs.enD_DATE = new Date(this.toDate!).toDateString();
