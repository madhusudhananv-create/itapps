...
// Find and replace the specific calculation section
const oldPattern = /\/\/ Calculate Total Avg CSAT Scores: Avg\(avg\(rating column value for each perspective\)\)[\s\S]*?const totalAvgCSATScores = perspectiveAverages\.length > 0[\s\S]*?\? '0\.00';/g;

const newCalculation = // Calculate Total Avg CSAT Scores: sum(Avg(Rating Value for "Overall Experience")) / count of perspective columns with non-zero ratings
...
content = content.replace(oldPattern, newCalculation);
