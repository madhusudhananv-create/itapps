...
export class ReportsComponent implements OnInit {
  ...
  // ── Wild (global) search across all report columns ──────────────

  /** Total number of rows currently loaded for the report */
  get totalRowCount(): number {
    return this.FinalTabData.length;
  }

  /** Number of rows matching the current wild search (equals totalRowCount when no search text) */
  get filteredRowCount(): number {
    return this.dataSource?.filteredData?.length ?? this.totalRowCount;
  }

  /** Apply the wild search text as a filter across every column of the report */
  applyGlobalFilter(): void {
    this.dataSource.filter = this.globalSearchText.trim().toLowerCase();
  }

  /** Clear the wild search box and reset the filter */
