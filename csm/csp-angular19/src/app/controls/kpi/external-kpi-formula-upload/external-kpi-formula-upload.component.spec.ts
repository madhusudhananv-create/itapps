...
describe('ExternalKpiFormulaUploadComponent', () => {
  ...
  const mockDialogData = { custId: 'CUST789' };
  ...
  describe('Component Initialization', () => {
    it('should initialize with dialog data', () => {
      expect(component.custId).toBe('CUST789');
    });
  ...
  describe('Modern UI Methods', () => {
    it('should reset file on source change', () => {
      const mockFile = new File(['test'], 'formulas.csv', { type: 'text/csv' });
    ...
    it('should handle file change event', () => {
      const mockFile = new File(['formula,value'], 'formulas.csv', { type: 'text/csv' });
      ...
      component.onFileChange(new Event('change'), mockFileInput);
  ...
  describe('validateDroppedFile', () => {
    ...
    it('should return false for file exceeding 16 MB', () => {
      const largeFile = new File(['x'], 'formulas.csv', { type: 'text/csv' });
      Object.defineProperty(largeFile, 'size', { value: 17 * 1024 * 1024 });

      const result = component.validateDroppedFile(largeFile);

      expect(result).toBe(false);
      expect(mockMyUtility.showWarning).toHaveBeenCalledWith('File size exceeds 16 MB limit');
    });

    it('should return false for wrong file type (CSV for FreshWorks)', () => {
      const wrongFile = new File(['test'], 'formulas.txt', { type: 'text/plain' });
      Object.defineProperty(wrongFile, 'size', { value: 1024 });

      const result = component.validateDroppedFile(wrongFile);

      expect(result).toBe(false);
      expect(mockMyUtility.showWarning).toHaveBeenCalledWith('Please select a CSV file');
    });

    it('should return false for wrong file type (JSON for ZIF)', () => {
      component.selSource = 'zif';
      const wrongFile = new File(['test'], 'formulas.csv', { type: 'text/csv' });
      Object.defineProperty(wrongFile, 'size', { value: 1024 });

      const result = component.validateDroppedFile(wrongFile);

      expect(result).toBe(false);
      expect(mockMyUtility.showWarning).toHaveBeenCalledWith('Please select a JSON file');
    });

    it('should return true for valid CSV file', () => {
      const validFile = new File(['formula,value'], 'formulas.csv', { type: 'text/csv' });
    ...
    it('should return true for valid JSON file', () => {
      component.selSource = 'zif';
      const validFile = new File(['{}'], 'formulas.json', { type: 'application/json' });
      Object.defineProperty(validFile, 'size', { value: 1024 * 1024 });
  ...
  describe('service_AddFile', () => {
    ...
    beforeEach(() => {
      mockFile = new File(['formula,value'], 'formulas.csv', { type: 'text/csv' });
    ...
    it('should show warning when no file is selected', () => {
      ...
      httpMock.expectNone(environment.webapiuri + 'UploadExternalKPIFormulas');
    });
    ...
    it('should show warning for wrong file type (not CSV for FreshWorks)', () => {
      const jsonFile = new File(['{}'], 'formulas.json', { type: 'application/json' });
      component.file = jsonFile;
      component.selSource = 'freshworks';
      
      component.service_AddFile();
      
      expect(mockMyUtility.showWarning).toHaveBeenCalledWith('Please select a CSV file for FreshWorks');
      httpMock.expectNone(environment.webapiuri + 'UploadExternalKPIFormulas');
    });
    ...
    it('should show warning for wrong file type (not JSON for ZIF)', () => {
      ...
      expect(mockMyUtility.showWarning).toHaveBeenCalledWith('Please select a JSON file for ZIF');
      httpMock.expectNone(environment.webapiuri + 'UploadExternalKPIFormulas');
    });
    ...
    it('should upload formula file successfully for FreshWorks', () => {
      component.file = mockFile;
      component.custId = 'CUST789';
      component.selSource = 'freshworks';

      const mockResponse = 'Formulas uploaded successfully';

      component.service_AddFile();

      expect(component._loading).toBe(true);

      const req = httpMock.expectOne(environment.webapiuri + 'UploadExternalKPIFormulas');
      expect(req.request.method).toBe('POST');
      expect(req.request.headers.get('empId')).toBe('12345');
      expect(req.request.headers.get('token')).toBe('test-token-456');
      expect(req.request.headers.get('customerId')).toBe('CUST789');
      expect(req.request.headers.get('source')).toBe('freshworks');
      expect(req.request.headers.get('fileType')).toBe('formula');
    ...
    it('should upload formula file successfully for ZIF', () => {
      const jsonFile = new File(['{"formulas": []}'], 'formulas.json', { type: 'application/json' });
      component.file = jsonFile;
      component.custId = 'CUST999';
      component.selSource = 'zif';

      const mockResponse = ['Formula upload completed'];

      component.service_AddFile();

      const req = httpMock.expectOne(environment.webapiuri + 'UploadExternalKPIFormulas');
      expect(req.request.headers.get('source')).toBe('zif');
      expect(req.request.headers.get('customerId')).toBe('CUST999');
      expect(req.request.headers.get('fileType')).toBe('formula');
    ...
    it('should use correct FormData field name', () => {
      component.file = mockFile;
    ...
    it('should set fileType header to "formula"', () => {
      component.file = mockFile;
  ...
  describe('Source Selection', () => {
    ...
    it('should validate based on selected source', () => {
      const csvFile = new File(['formula,value'], 'formulas.csv', { type: 'text/csv' });
  ...
  describe('Component Rendering', () => {
    it('should display the dialog title', () => {
      ...
      expect(titleElement.textContent.trim()).toBe('Upload KPI Formulas');
    });
    ...
    it('should have select placeholder option', () => {
      ...
      expect(options[0].textContent.trim()).toContain('Select a source');
    });
  ...
  describe('Integration Tests', () => {
    it('should complete full upload workflow for FreshWorks formulas', () => {
      const mockFile = new File(['formula,value\ntest,123'], 'formulas.csv', { type: 'text/csv' });
      Object.defineProperty(mockFile, 'size', { value: 1024 * 1024 });
      ...
      req.flush('Formula upload completed successfully');

      expect(component._loading).toBe(false);
      expect(mockMyUtility.showSuccess).toHaveBeenCalledWith('Formula upload completed successfully');
      expect(mockDialogRef.close).toHaveBeenCalled();
    });

    it('should complete full upload workflow for ZIF formulas', () => {
      const mockFile = new File(['{"formulas": [{"name": "test"}]}'], 'formulas.json', { 
  ...
  describe('API Endpoint', () => {
    it('should use correct API endpoint for formula upload', () => {
      const mockFile = new File(['test'], 'formulas.csv', { type: 'text/csv' });
      component.file = mockFile;
      component.selSource = 'freshworks';
      ...
      const req = httpMock.expectOne(environment.webapiuri + 'UploadExternalKPIFormulas');
      expect(req.request.url).toBe(environment.webapiuri + 'UploadExternalKPIFormulas');
