...
import { MatDialogConfig, MatDialog, MatSelect, MatDialogModule } from '@angular/material';
...
import { ProductKpiDetailsComponent } from './product-kpi-details/product-kpi-details.component';
import { TrendHighChartComponent } from '../../successgoal/trend-high-chart/trend-high-chart.component';
import { AchievementtrendComponent } from '../../dashboard/dashboard-customer/Achievementtrend/Achievementtrend.component'
...
export class DashboardCustomerComponent extends basepage implements OnInit {
  sMonth: string = '';
  iYear: string = '';
  cColor: string;
  vColor: string;
  ...
  showtooltip: boolean = false; findingdatatype: any[] = [];
  productScores: any[];
  engagementKPI: any[];
  ...
  currIndex: number = 0;
  @ViewChild(DashboardCustomerNextPageComponent) chviewChild: DashboardCustomerNextPageComponent;
  result: any;
  ...
  fillQAFindingsByTime1() {
    let result = [' 7 days', '> 7 days', '> 14 days', '> 21 days', '> 30 days'];
    let valuesArray = [0];
    for (let i = 0; i < result.length; i++) {
      for (let j = 0; j < this.projArray.length; j++) {
        if (isNaN(valuesArray[i]))
  ...
  filterStaffingAndBillingSummary1() {
    ...
    this.projArray.forEach(x => {
      ...
      offshoreNonBillable = offshoreNonBillable + this.getGraphValue_project('OFFSHORE_NON_BILLABLE', x);
    });
  ...
  service_LoadCustomerByEmpId() {
    this._appservice.GetCustomerList(localStorage.getItem('empid'), false).subscribe(data => {
      ...
      if (this.customerList.length > 0) {
        ...
        if (this._util.IsPremier(this.customerid) || this.isBaseMeasureEnabled) {
          this._router.navigateByUrl('/serviceleveldashboard/cust/212100001/true');
        }
        else {
          this._router.navigateByUrl('/newdashboard/cust/' + this.customerid.toString() + '/false');
        }
  ...
  getTitleByPortfolio(title, portfolio) {
    ...
    // if (this.projectHealthHigh != undefined && title == "PROJECT_HEALTH_H") {
    //   if (title == "PROJECT_HEALTH_H" && portfolio == 1)
    //     content = this.projectHealthHigh[0].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_H" && portfolio == 2)
    //     content = this.projectHealthHigh[1].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_H" && portfolio == 3)
    //     content = this.projectHealthHigh[2].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_H" && portfolio == 4)
    //     content = this.projectHealthHigh[3].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_H" && portfolio == 5)
    //     content = this.projectHealthHigh[4].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_H" && portfolio == 6)
    //     content = this.projectHealthHigh[5].healtH_SCORE;
    // }

    // if (this.projectHealthMed != undefined && title == "PROJECT_HEALTH_M") {
    //   if (title == "PROJECT_HEALTH_M" && portfolio == 1)
    //     content = this.projectHealthMed[0].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_M" && portfolio == 2)
    //     content = this.projectHealthMed[1].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_M" && portfolio == 3)
    //     content = this.projectHealthMed[2].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_M" && portfolio == 4)
    //     content = this.projectHealthMed[3].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_M" && portfolio == 5)
    //     content = this.projectHealthMed[4].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_M" && portfolio == 6)
    //     content = this.projectHealthMed[5].healtH_SCORE;
    // }

    // if (this.projectHealthLow != undefined && title == "PROJECT_HEALTH_L") {
    //   if (title == "PROJECT_HEALTH_L" && portfolio == 1)
    //     content = this.projectHealthLow[0].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_L" && portfolio == 2)
    //     content = this.projectHealthLow[1].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_L" && portfolio == 3)
    //     content = this.projectHealthLow[2].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_L" && portfolio == 4)
    //     content = this.projectHealthLow[3].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_L" && portfolio == 5)
    //     content = this.projectHealthLow[4].healtH_SCORE;
    //   else if (title == "PROJECT_HEALTH_L" && portfolio == 6)
    //     content = this.projectHealthLow[5].healtH_SCORE;
    // }
  ...
  loadSuccessGoalForPeriod(bLastUpdated: boolean) {
    if (this._util.IsPremier(this.selectedCustomer.cusT_ID)) {
      this._appservice.getServiceMetricsForAPeriod(this.selectedCustomer.cusT_ID, this._dashboardUtil.filteR_MONTH, this._dashboardUtil.filteR_YEAR, bLastUpdated).subscribe(
        data => {
          this.healthScoresOverall = data.portfoliO_WISE_KPI;
          this.productScores = data.producT_WISE_KPI;
          this.engagementKPI = data.engagemenT_WISE_KPI;
          ...
          this.healthScoresOverall.forEach(ele => {
            this.countOfProds = this.countOfProds + ele.producT_COUNT;
    ...
    //else{
    this._appservice.getSuccessGoalScoreForAPeriod(this.selectedCustomer.cusT_ID, this._dashboardUtil.csG_FILTER_MONTH, this._dashboardUtil.csG_FILTER_YEAR, bLastUpdated).subscribe(
      data => {
    ...
    this.GetProjectCAPACount(this._dashboardUtil.csG_FILTER_MONTH, this._dashboardUtil.csG_FILTER_YEAR);    // }
  }
  temp: any = [];
  ...
  fillQAComplianceScoreByProcessModel() {
    let titles = []
    titles = this.gettitlesbystringForCustomer('COMPLIANCE_SCORE_', this.selectedCustomer.cusT_ID);
  ...
  fillQAFindingsByStage() {
    ...
    //let result = [];
    let titles = []
    titles = this.gettitlesbystringForCustomer('STAGE_FINDING_', this.selectedCustomer.cusT_ID);
    titles = titles.filter((x, i, a) => a.indexOf(x) == i);
  ...
  fillQAFindingsSummary() {
    let titles = [];
    titles = this.gettitlesbystringForCustomer('FINDING_', this.selectedCustomer.cusT_ID);
    titles = titles.filter((x, i, a) => a.indexOf(x) == i);
    let values = ["0"];
    var title = "";
  ...
  widthfindingstage = 180;
  heightfindingstage = 90;
  ...
  serviceType = 'PieChart';
  serviceWidth = 180;
  serviceHeight = 150;
  servicelvlCol = ['Service Level Measurement Type', 'count'];
  servicelvlData = [
    ['Key Measurement', 19],
    ['Critical Measurement', 12]

  ];
  servicelvlOpt: google.visualization.PieChartOptions = {
  ...
  openAchievementtrendScreen(proJ_ID) {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.autoFocus = true;
    dialogConfig.data = {
      'custid': this.selectedCustomer.cusT_ID,
  ...
  GetProjectCAPACount(month, year) {
    this._appservice.GetProjectCAPACount
      (this.selectedCustomer.cusT_ID, month, year).subscribe(
        data => {
