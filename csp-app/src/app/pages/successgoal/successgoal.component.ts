...
import { MatDialogConfig, MatDialog, MatPaginator, MatSort, MatTableDataSource, MatSelect } from '@angular/material';
import { ChartsService } from '../../Services/charts.service';
import { Ng4LoadingSpinnerService } from 'ng4-loading-spinner';
import { SelectionModel } from '@angular/cdk/collections';
import { SLARejectionDetails } from '../../models/kpi-details-extended-model';
...
export class SuccessgoalComponent implements OnInit {
  ...
  selection = new SelectionModel<SLARejectionDetails>(true, []);
  selectionKPI = new SelectionModel<SLARejectionDetails>(true, []);
  ...
  capaStageId: any;
  employeeRole: any;
  constructor(private route: ActivatedRoute, private _chartsService: ChartsService, changeDetectorRef: ChangeDetectorRef, media: MediaMatcher, private _appService: AppsService, public _util: myUtility, public dialog: MatDialog, private _spinner: Ng4LoadingSpinnerService, private router: Router, public _access: AccessControl) {
  ...
  ngOnInit() {
    ...
    this.slaAvailable = slaAvailableList.length > 0 && slaAvailableList.filter(x => x.customerId == this.input_custId)[0].slaAvailable;
    ...
    if (this.slaAvailable && this.prodId != undefined && !this.isDraft) {
      ...
      if (confirm("Are you sure want to revert for " + this.tableMonth + ' ' + this.tableYear)) {
        this._appService.revertProductKPIDetails(this.prodId, this.tableMonth, this.tableYear).subscribe(data => {
  ...
  getTrendHighChartDetails() {
    ...
    if (this._util.IsPremier(this.input_custId) && this.prodId != undefined) {
      this._chartsService.getTrendHighChartDetailsForProductKPI(this.input_custId, this.prodId, date, this._util.AppSettings.token, this.viewBy).subscribe(
        data => {
          if (data.length > 0) {
  ...
  displayGraph(kpiid, goalname) {

    let selectedGraph = this.trendHighChart.filter(x => x.goalName == goalname.split('|')[0])[0].trendHighChart.filter(x => x.kpiId == kpiid)[0].trendHighChart;
  ...
  GetTable(client_ID, proj_ID, date) {
    ...
    this._chartsService.getTableSuccess(this.selGroupBy, client_ID, proj_ID, date, 'Monthly', (serviceTowers == "null" ? null : serviceTowers), this.AppSettings.token)
      .subscribe
      (
        data => {
  ...
  getSecondarySLAStatus(definitions) {
    if (this.includeExclusions) {
    ...
    this._appService.getKpiMetrics(this.prodId, this.modeId, d, false).subscribe(
      async data => {
        ...
        if (this.flagValue == 'true' && this.capaStageId == -1) {
          ...
          this.metricsDetail = data.filter(x => x.capA_STAGE_ID == this.capaStageId);
  ...
  loadOverallDetails() {
    ...
    this._appService.getOverallServiceMetricsForAPeriod(this.input_custId, this.tableMonth, this.tableYear).subscribe(
      data => {
        this.productScores = data.filter(x => x.producT_ID == this.prodId);
  ...
  getmeasurementforServiceLevel(kpiId) {
    ...
    if (this.kpiDetail.length > 0) {
      if (kpiId != undefined || kpiId != null) {
        uom = this.kpiDetail.filter(x => x.kpI_ID == kpiId)[0]!.uniT_OF_MEASUREMENT;
        expectedLvl = this.kpiDetail.filter(x => x.kpI_ID == kpiId)[0]!.expecteD_SERVICE_LEVEL;
        if (uom == '%')
          return expectedLvl + '%'
        else if (uom == 'Number')
  ...
  changeGroupBy(headerName, selGroupBy): void {
  ...
  getmeasurementforMinServiceLevel(kpiId) {
    ...
    if (this.kpiDetail.length > 0) {
      if (kpiId != undefined || kpiId != null) {
        uom = this.kpiDetail.filter(x => x.kpI_ID == kpiId)[0]!.uniT_OF_MEASUREMENT;
        expectedLvl = this.kpiDetail.filter(x => x.kpI_ID == kpiId)[0]!.minimuM_SERVICE_LEVEL;
        if (uom == '%')
          return expectedLvl + '%'
        else if (uom == 'Number')
          return expectedLvl + ' per product'
  ...
  filterData() {
    let serviceLevelStatusFilter = this.filterCriteria.filter(x => x.displaY_NAME == 'Service Level Status')[0];
  ...
  ViewCAPA(id) {
    ...
    kpiData = this.metricsDetail.filter(x => x.kpI_ID == id)[0];
  ...
  Service_GetServiceAreaProjectMapping() {
    ...
    if (this.input_projId != undefined) {
      this._appService.getServiceTowersProjectMapping(this.input_projId).subscribe(data => {
  ...
  updateSLARejection() {
    ...
    if (this.isValidInputs()) {
      if (confirm("Are you sure want to update SLA Rejection for the selected SLA(s)?")) {
  ...
  rejectSLA() {
    let date: String = (this.tableYear + '-' + this.tableMonth + '-01');
    if (this.isValidInputs()) {
      if (confirm("Are You sure You want to reject selected SLA(s)?")) {
        let selectedSLA = this.selection.selected;
        let selection = selectedSLA.filter(x => x.slA_Rejection_data);
        let rejectedSLA: any = [];
        if (selectedSLA.length > 0) {
  ...
  getEmployeeRolesForProduct() {
    this.isLoading = true;
    this._appService.getEmployeeRolesForProduct(this.prodId).subscribe(data => {
      this.employeeRole = data;
      this.isLoading = false;
    }, (err) => { this._util.serviceError(err); this.isLoading = false; })
  }

  sendSLAToCustomer() {
    this.isLoading = true;
    let date: String = (this.tableYear + '-' + this.tableMonth + '-01');
    if (confirm("Are You sure You want to send all SLA metrics to the customer?")) {
      this.isLoading = false;
      this._appService.sendKPIDetailsReviewFeedback(this.prodId, date).subscribe(data => {
        this.isLoading = false;
  ...
  sendReviewFeedback() {
    ...
    if (this.isValidInputsForReviewFeedback()) {
      if (confirm("Are You sure You want to submit the Review Comments for the selected SLA(s)?")) {
        let selectedSLA = this.selection.selected;
        let reviewedSLA: any = [];
        let productId: any;
        if (selectedSLA.length > 0) {
  ...
  submitSLAForRejections(slaRejectionData: any, date, msg) {
    this.isLoading = true;
    this._appService.updateSLARejection(slaRejectionData, date).subscribe(data => {
      this.isLoading = false;
      this.reloadProdKPITable();
  ...
  sendForReviewFeedback(reviewedSLA: any, productId, date, msg) {
    this.isLoading = true;
    this._appService.sendReviewFeedback(reviewedSLA, productId, date).subscribe(data => {
      this.isLoading = false;
      this.reloadProdKPITable();
      alert(msg); this.clear();
    }, (err) => { this._util.serviceError(err); this.isLoading = false; })
