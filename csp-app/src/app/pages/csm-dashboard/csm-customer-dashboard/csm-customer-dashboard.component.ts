...
import { ProjectModelNew, PortfolioModel } from '../../../models/portfolio-model';
import { SuccessGoalsScoresModel, DashboardDetailsModel, SuccessGoalsScoresModelForAProject } from '../../../models/dashboard-details-model';
...
import { IssueProgressStatusComponent } from './issue-progress-status/issue-progress-status.component';
...
export class CsmCustomerDashboardComponent extends basepage implements OnInit {
  ...
  constructor(public _dashboardUtil: CSMDashboardService, private route: ActivatedRoute, private _router: Router,
    public _access: AccessControl, private _appservice: AppsService, public _util: myUtility, changeDetectorRef: ChangeDetectorRef,
  ...
  applyFilter() {
    ...
    if(this._util.IsPremier(this.customerId)){
...
loadAccountHealth(){
  ...
  this.params.projIds = this.projID;
  //this.customerId,this.projID,date.getMonth() +1, date.getFullYear()
  this._appservice.getAllAccountHealth(this.customerId,this.params.projIds,this.params.month,this.params.year).subscribe(
    data => {
        this.needControl = data.need_Control;
        this.underControl = data.under_Control;
        console.log(this.needControl);
        this.OntabChange(0);
    },(error) =>{this._util.serviceError(error);}
  )
}
  loadGauge(){
    ...
    let custIds : string[]=[];
    custIds.push(this.customerId.toString());
    this.requestObj.startDate = new Date(this.fromDate).toDateString();
    this.requestObj.endDate = new Date(this.toDate).toDateString();
    this.requestObj.globalkpis = this.globaL_KPI_CATEGORY_IDs.map((x) => x.toString());
    this.requestObj.customerids = custIds;
    this.requestObj.projectids = this.projID;
    this.requestObj.initialGroup = 'SUMMARY'
    this._appservice.GetConsolidatedProjectWiseKPIDetails(this.requestObj).subscribe(
      data => {
        this.processedData = data.overallkpi;
        this.gaugeData = [];
        for(var row of this.processedData){
          this.gaugeData.push(['GOAL',row.cusT_DATAS.kpI_ACHIEVED])
        }
        console.log("summary data", this.processedData);
      },(error) =>{this._util.serviceError(error);}
    )
  }
  ...
  valueBar:string
  loadSuccessBar(){
    this.performanceBar = this.getTitleByCustomer('SUCCESS_GOAL_SCORE_PERFORMANCE');
    this.performanceBar = this.performanceBar.replace("%","");
    this.qualityBar = this.getTitleByCustomer('SUCCESS_GOAL_SCORE_QUALITY');
    this.qualityBar = this.qualityBar.replace("%","");
    this.compilanceBar = this.getTitleByCustomer('SUCCESS_GOAL_SCORE_COMPLIANCE');
    this.compilanceBar = this.compilanceBar.replace("%","");
    this.valueBar = this.getTitleByCustomer('SUCCESS_GOAL_SCORE_VALUE');
    this.valueBar = this.valueBar.replace("%","");
  }
  ...
  // getAccountHealthCountforProject(title,index){
  //   let content: number=0;
  //   let count =[];
  //   if(index == 0){
  //     if(this.needControl.length > 0){
  //     count = this.needControl.filter(x => x.projecT_COUNT == title);
  //     if(count.length > 0){
  //       content = count[0].projecT_COUNT;
  //     }
  //     }
  //   }
  //   return content;
  // }
  // getAccountHealthCountforPort(title,index){
  //   let content: number=0;
  //   let count =[];
  //   if(index == 0){
  //     if(this.needControl.length > 0){
  //     count = this.needControl.filter(x => x.portfoliO_COUNT == title);
  //     if(count.length > 0){
  //       content = count[0].portfoliO_COUNT;
  //     }
  //     }
  //   }
  //   return content;
  ...
  dataArea1 : any[]=[];
  loadActionItemArea() {
    this._appservice.getAreaActionItem(this.projID).subscribe(data => {
     // this.dataArea1 = [];
  ...
  loadRiskItemArea() {
    this._appservice.getRiskAreaItem(this.projID).subscribe(data => {
      this.riskDataArea = [];
  ...
  loadIssueItemArea() {
    this._appservice.getIssueAreaItem(this.projID).subscribe(data => {
      this.issueDataArea = [];
  ...
  loadTeamCount(){
    this._appservice.getProjectTeamCount(this.projID).subscribe(data => {
      this.teamSize = data;
  ...
  filterProjectList1() {
    ...
    if (this.projID.length > 0 && this.projectScores != undefined)
      this.projectScores = this.projectScores.filter(f => this.projID.includes(f.proJ_ID));
  ...
  service_getPortfolioDetails() {
    this._appservice.GetPortfolioList().subscribe(data => {
      ...
      //console.log("Portfolio:",this.portfolioList)
    }, error => { this._util.serviceError(error); });
  }
...
getproject_TeamCount(title,x){
  ...
  filterProjecttoStart(){
    ...
    // if (this.dashboardDetails != undefined){
    //   let details: DashboardDetailsModel[] = [];
    //   details = this.dashboardDetails.filter(t => t.title == 'PROJECT_TO_START' && t.cusT_ID == this.customerId && t.proJ_ID == this.projID);
    //   if (details.length > 0) {
    //     this.projectToStart = details[0].content;
    //   }
    // }
  ...
  filterProjecttoEnd(){
    ...
    // if (this.dashboardDetails != undefined){
    //   let details: DashboardDetailsModel[] = [];
    //   details = this.dashboardDetails.filter(t => t.title == 'PROJECT_TO_END' && t.cusT_ID == this.customerId);
    //   if (details.length > 0) {
    //     this.projectToEnd = details[0].content;
    //   }
    // }
  ...
  loadSuccessGoalForPeriod(bLastUpdated: boolean,fromDate,toDate) {
    this._appservice.getSuccessGoalScoreForAPeriodNew(this.selectedCustomer.cusT_ID, fromDate, toDate, bLastUpdated).subscribe(
      data => {
        console.log("DATA:",data);
        this.projectScores = data.projecT_SCORES;
        this.tempScoresArray = data.projecT_SCORES;
        //console.log("Scores:",this.projectScores)
        ...
        this._dashboardUtil.CSG_FilterMonth = data.month;
        this._dashboardUtil.CSG_FilterYear = data.year;
  ...
  //   this._appservice.getSuccessGoalScoreForAPeriod(this.selectedCustomer.cusT_ID, this._dashboardUtil.CSG_FilterMonth, this._dashboardUtil.CSG_FilterYear, bLastUpdated).subscribe(
  //     data => {
  //       console.log("DATA:",data);
  //       this.projectScores = data.projecT_SCORES;
  //       this.tempScoresArray = data.projecT_SCORES;
  ...
  loadRiskHeatMap(){
    this._util.GetCharts(this.customerId,this.projID);
  }
  openKPItrendScreen() {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.autoFocus = true;
  ...
  openRiskPopUp(){
    console.log("Risk Heat Map:",this._util.chartsMonthly.riskChart.data)
    const dialogConfig = new MatDialogConfig();
    dialogConfig.autoFocus = true;
    dialogConfig.data = {
      'input':this._util.chartsMonthly.riskChart.data,
      'customerId': this.selectedCustomer.cusT_ID
