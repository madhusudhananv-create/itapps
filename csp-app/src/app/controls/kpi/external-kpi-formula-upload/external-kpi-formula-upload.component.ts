...
@Component({
  ...
  styleUrls: ['./external-kpi-formula-upload.component.scss']
})

export class ExternalKpiFormulaUploadComponent implements OnInit {
  custId:string ;
  constructor(private dialog: MatDialogRef<ExternalKpiFormulaUploadComponent>, @Inject(MAT_DIALOG_DATA) public data: any,private _util: myUtility, private _http: Http) { 
  ...
  service_AddFile(fileName) {
    ...
    if (fileList.length > 0) {
      ...
      headers.append('fileType', 'formula');
